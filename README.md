...
The three research pages share tokens and components so they read as one set, while each keeps its own structure: this page's purple Related Center Programming band and multi-select theme chips, the compendium's charts and boxed summaries, grants-overview's grant cards. Shared: the accent palette and light tag style, `--radius-card` 6px / `--radius-control` 4px, serif 600 titles for works (paper titles here), controls straight on the light-gray filter strip, category 2 "Read paper ⟶" links, and the white gradient-topped card for the key summary text. Change one of these here and check the other two.
