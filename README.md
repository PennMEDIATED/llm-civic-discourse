...
## Shared with research-compendium and grants-overview

The three research pages share tokens and components so they read as one set, while each keeps its own structure: this page's three-per-row paper cards, the Related Center Programming stack beside the intro and multi-select theme chips, the compendium's charts and boxed summaries, grants-overview's grant cards. Shared: the accent palette and light tag style, `--radius-card` 6px / `--radius-control` 4px, serif 600 titles for works (paper titles here), controls straight on the light-gray filter strip, category 2 "Read paper ⟶" links, and the white gradient-topped card for the key summary text. Change one of these here and check the other two.

## Editing a paper

Copy an existing object in the `papers` array (`id`, `authors`, `year`, `title`, `url`, `journal`, `summary`, `tags`; `new`/`pinned` are optional flags on individual entries but neither is currently read by any sort/filter/render logic — they're bookkeeping for whoever edits the data, not functional). `tags` must be drawn from (or added to) `TAG_COLORS` just below the array, which is both the master tag list and each tag's color — order in `TAG_COLORS` also sets the display order of the theme filter chips. `"Penn Research"` is a special tag: excluded from the inline theme-tag row on each card, it is shown instead as a "Penn Research" corner label on the card (restored 2026-09 with the three-per-row card grid, in the Penn Research chip's lilac), it still pins the paper to the top of the default (year-desc) sort and is filtered by the "Penn Research" chip in the Themes row like any other tag.
...
**Tag/chip design**: theme tags — the filter-row chips and each card's small tags — use the sitewide accent palette (see `research-compendium`'s README "Accent palette") in the light tag style: tint fill, 1px border in the solid colour, dark text, 4px corners. `TAG_COLORS` in `vendor/app.js` holds each theme's `bg` (tint) / `border` (solid) / `text`. A theme that means the same thing on another research page keeps that page's colour: Misinformation = orange-red, Persuasion = magenta, Political Bias = purple. Filter chips escalate the border for state (1.5px at rest; 2.5px plus a soft 4px glow in the tag's colour when active); on-card tags use a plain 1px border.
...
- **Filter row** — the controls (search with its magnifying-glass icon, sort `<select>`, Clear filters) sit directly on the full-width `--c-light-bg` strip with no inner white box, the same as `research-compendium`. Fields are white with 4px corners (`--radius-control`).
...
- **Paper cards** — 6px corners (`--radius-card`, matching home's cards), the shared `0 12px 24px rgba(13,13,12,.16)` + `translateY(-4px)` hover lift. A column header row (`.papers-head`: Paper / Authors / Year / Summary) sits above the list on the same grid and hides under 900px. Opening a card shows a light-gray area holding a white **Summary** card with the brand-gradient top edge, then a category 2 **Read paper ⟶** link — the same treatment as the compendium's "Why is this important?" card and paper link.
- **Header** (2026-09) — no colour band. Left: the purple serif title and the intro, 80px below the site menu like every page. Right: an orange "Related Center Programming" eyebrow (same treatment as the compendium's "Center newsletter" label) over three stacked light-gray cards: the convening (topic primer PDF), the 2026 grants (the four titles sit behind a "See the four grants" toggle, closed by default, to keep the card short) and the Carnegie Endowment report on monitoring LLMs. The two columns stack under 900px. The point is that the filter strip and the first row of papers sit within the first screen. The old purple "stats band" that held the two programming tiles is gone.
- **Paper cards** (2026-09) — back to the original page's grid: three per row, two under 1100px, one under 700px. Each card: journal (uppercase) and year, serif title (links to the paper), authors, summary clamped to five lines, theme tags and a chevron that expands the summary in place. Clicking anywhere on the card also toggles it. The column-header bar (Paper / Authors / Year / Summary) and the row layout it labelled are gone. Card text is the pre-redesign page's, word for word.
...
```html
<iframe id="pm-llm-civic-discourse" src="https://pennmediated.github.io/llm-civic-discourse/" title="LLMs & Civic Discourse — Penn MEDIATED" scrolling="no" loading="lazy" style="width:100%;height:12500px;border:0;display:block"></iframe><script>(function(){var f=document.getElementById('pm-llm-civic-discourse');window.addEventListener('message',function(e){if(e.source!==f.contentWindow)return;var d=e.data||{},h=d.frameHeight||(d.type==='partners-page-resize'?d.height:0);if(h)f.style.height=h+'px';});})();</script>
```

The `height` in the snippet is only the starting value. For this page it matters more than most: it renders through Babel in the browser, so the frame sits at the starting value for a noticeable beat before the real height arrives. Measured 2026-09-22 at 1512px wide, the real content is 12,683px — the starting value is deliberately set just under that, at 12,500px, so the frame grows the last couple of hundred pixels into place rather than collapsing from a tall blank box. It was 16,000px, an estimate that left roughly 3,300px of white. Re-measure with `document.body.scrollHeight` if the entry list changes substantially. Every Penn MEDIATED page posts its real height to the parent as `{ frameHeight: <int> }` — on load, on resize, once webfonts settle, and on any `ResizeObserver` change, so reveal animations, expanding cards and `<details>` toggles all resize the frame. The listener in the snippet applies it. `grants-rfp` also emits an older `{ type: 'partners-page-resize', height }` message; the snippet accepts both.
